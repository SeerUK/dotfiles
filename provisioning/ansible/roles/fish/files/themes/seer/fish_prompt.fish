...
function __bobthefish_prompt_git -d 'Display the actual git state'
  ...
  set -l ahead   (command git rev-list --left-right '@{upstream}...HEAD' ^/dev/null | awk '/>/ {a += 1} /</ {b += 1} {if (a > 0) nextfile} END {if (a > 0 && b > 0) print "±"; else if (a > 0) print "+"; else if (bb > 0) print "-"}')
  ...
  __bobthefish_start_segment fff $flag_fg
  ...
  if [ "$project_pwd" ]
    if [ -w "$PWD" ]
      __bobthefish_start_segment fff magenta
    else
      __bobthefish_start_segment fff red
    end
